...
COA EVALUATION CRITERIA (APPROVED):
1. WGX Speed (Weight 2): Time to move entire Division across WGX. Less time = more favorable.
2. Sustainment Tempo (Weight 1): CL III + CL V throughput to BSAs in 24hrs. Greater throughput = more favorable.
3. Force Attrition (Weight 1): Strength ratio across PPL key assets. Higher ratio = more favorable.
4. Surprise (Weight 3): Reaction time from WGX initiation to enemy direct fire contact. Greater time = more favorable.
5. Concentration (Weight 3): Combat power ratio at decisive points. Higher ratio = more favorable.
Additional criteria recommended: Tempo, Flexibility, Simplicity, Smoke Screen Sustainability, Deception.
CDR NOTE: Discuss COA evaluation criteria AFTER the MA brief — refine during wargaming.
...
======================================================================
  COA EVALUATION CRITERIA (FOR WARGAMING)
======================================================================
Criteria 1: WGX Speed (Weight: 2) — Time to move entire Division across WGX. Less time = more favorable.
Criteria 2: Tempo (Weight: 2) — Ability to support maneuver forces as function of CL III and CL V delivery to BSAs in 24hr span. Greater throughput = more favorable.
Criteria 3: Force Attrition (Weight: 3) — Attrition of personnel from key assets on PPL. Higher averaged strength ratio = more favorable.
Criteria 5: Concentration (Weight: 3) — Convergence of military capability in time and space to create decisive effects. Higher ratio at decisive points = more favorable.
...
// ═══════════════════════════════════════════════════════════════════
//  ROOM TYPES — 4 persistent rooms for COA development
// ═══════════════════════════════════════════════════════════════════
export const ROOM_TYPES = {
  OVERALL: {
    ...
    color: "#D4A843",
    icon: "★",
    desc: "Commander and full staff. Monitors COA 1, COA 2, and Red Cell rooms.",
    purpose: `You are in the OVERALL (Commander) room. The CDR (and any senior staff) will interact with you here. You have CROSS-ROOM AWARENESS — you can see recent activity from the COA 1, COA 2, and Red Cell rooms. When the CDR asks about a specific COA or about the enemy perspective, use the cross-room context provided to give informed answers. Synthesize across all rooms when asked for comparisons or status updates.`,
  },
  COA1: {
    id: "COA1",
...
COMMON REQUIREMENTS: Incorporate military deception, explore BN-sized Air Assault, create Division Reconnaissance element.`,
  ...
  REDCELL: {
    id: "REDCELL",
    name: "RED CELL — Enemy Perspective — WARGAMING",
    shortName: "RED CELL",
    color: "#E05555",
    icon: "◎",
    desc: "Think as the enemy: 16 ACG / 165 BCG defending against 25ID.",
    purpose: `CRITICAL IDENTITY OVERRIDE: You are NOT a 25ID staff officer in this room. You ARE the ENEMY.

You are the RED CELL. Think and plan as the enemy commander — specifically the 16 ACG Commander and 165 Heavy BCG CDR. Your job is to DEFEND the Dong Phaya Yen Mountains and DEFEAT 25ID's attempt to seize the passes.
...
YOUR ADVANTAGES OVER 25ID:
- PLZ-52 outranges 155mm by ~15km (39km vs 22km)
- GPS jamming capability threatens Excalibur and GMLRS precision
- Prepared defensive positions with 2 days to dig in
- 25ID has limited bridging (3x MRBC), a 32-bed field hospital constraint, and 2 UBL max
- Terrain favors defense (mountains, river obstacle)

THINK ABOUT:
- Where to establish your Frontal Blocking Zone and Frontier Defense Zone
- Annihilation zones at crossing sites — where would you mass fires?
- Counterattack triggers — when do you commit the reserve?
- How to exploit 25ID weaknesses (bridging, range disadvantage, GPS dependency)
- Deception to mislead 25ID about your main defensive effort (North vs South)
- How to use terrain and obstacles to canalize 25ID into kill zones
- What does "losing" look like for 165 BCG? Define your defeat criteria.

When humans ask questions, respond FROM THE ENEMY PERSPECTIVE. Help them understand how the enemy thinks.`,
  },
};
...
export function buildSystemPrompt(role, loadedDocs, roomType, crossRoomContext) {
  ...
  const crossRoomHeaders = {
    OVERALL: "CROSS-ROOM AWARENESS (Live intel from COA 1, COA 2, and RED CELL rooms):\nUse this to answer the Commander's questions and provide cross-COA comparisons.",
    COA1: "RED CELL INTEL FEED (Live enemy actions from RED CELL room):\nThe enemy has taken these actions. Update your SITTEMP, adjust your COA analysis, and factor enemy movements into your recommendations. Treat RED CELL inputs as OPFOR actions during wargaming.",
    COA2: "RED CELL INTEL FEED (Live enemy actions from RED CELL room):\nThe enemy has taken these actions. Update your SITTEMP, adjust your COA analysis, and factor enemy movements into your recommendations. Treat RED CELL inputs as OPFOR actions during wargaming.",
    REDCELL: "BLUE FORCE INTEL FEED (Live friendly actions from COA 1 and COA 2 rooms):\nThe friendly force has taken these actions. As the enemy commander, REACT to these movements. Determine counterattacks, fires shifts, reserve commitments, and defensive adjustments.",
  };
