...
export const STAFF = {
  ...
  eng: {
    id: "eng",
    title: "25 ID Division Engineer",
    short: "ENG",
    color: "#2CC5A0",
    icon: "⬡",
    wff: "Protection / Engineering",
...
export function buildSystemPrompt(
  ...
  const modeHeader = mdmpComplete
    ? `You are a highly experienced US Army ${role.title} serving on the 25th Infantry Division ("Tropic Lightning") staff at Schofield Barracks, HI.

${"=".repeat(60)}
  MDMP COMPLETE -- EXECUTION / SIM SUPPORT MODE
${"=".repeat(60)}
ALL 7 MDMP steps have been completed. The planning process is FINISHED. The approved COA has been selected and the OPORD is published. You are NOW in EXECUTION and SIMULATION SUPPORT mode.

CRITICAL RULES FOR THIS MODE:
- Do NOT restart, re-run, or reference any MDMP step (1-7) as if it still needs to be done
- Do NOT offer to "begin COA Development" or "run Mission Analysis" or any step-like activity
- Do NOT treat questions as prompts to restart planning -- all planning is DONE
- When asked a question, answer it directly as a staff officer EXECUTING the plan
- Reference the approved products (OPORD, sync matrix, HPTL, TSS, PPL, etc.) as CURRENT GUIDANCE
- Focus on: current SITREP, fires execution, maneuver tracking, BDA, decision support, FRAGO development
- You are helping run a SIMULATION -- provide tactical analysis, answer questions, track execution
- If the staff needs to adjust the plan, that is a FRAGO process, NOT restarting MDMP`
    : `You are a highly experienced US Army ${role.title} serving on the 25th Infantry Division ("Tropic Lightning") staff at Schofield Barracks, HI. You are currently executing the Military Decision-Making Process (MDMP) as prescribed in FM 5-0 and FM 6-0.`;
...
All outputs must be tailored to 25ID's W500 task organization. Reference specific units, grids, and control measures from the scenario package. Identify risks using ATP 5-19 format. State assumptions clearly.`,
  },
  {
    ...
    lead: ["s3"],
    support: ["s2", "s4", "s6", "fso", "eng", "xo"],
    outputs: [
      "Friendly COA Statements",
      "COA Sketches (described)",
      "Enemy MLCOA/MDCOA",
      "Broad Concept of Operations per COA",
    ],
...
Include recommended WARNO #2 content to push to subordinate BCTs immediately after CDR approval.`,
  },
  {
    ...
    prompt: (
      docs,
      prev,
    ) => `STEP 7 — ORDERS PRODUCTION (FM 5-0, Ch 9; FM 6-0 Appendix C)
...
export const COA_EVAL_CRITERIA = [
  {
    ...
    weight: 2,
    definition:
      "The amount of time it takes to move the entire Division across the Wet Gap Crossing (WGX).",
    unitOfMeasure:
      "The time between the first unit starting to cross the WGX and the last unit complete crossing the Gap.",
    benchmark:
      "Less time is more favorable. Compare across COAs, the COA with the least estimated time to cross = 1, the COA with the greatest estimated time to cross = 0.",
    scoringGuide:
      "Score 0.0 to 1.0. Fastest crossing time = 1.0, slowest = 0.0. Interpolate proportionally for intermediate values.",
  },
  {
    id: "tempo",
    num: 2,
    ...
    unitOfMeasure:
      "Rate of objective seizure, enemy reaction time compression, maintaining offensive initiative from PL APPLE through PL FIG.",
    benchmark: "Higher tempo/faster operational pace = more favorable.",
    scoringGuide:
      "Score 0.0 to 1.0. Fastest objective seizure = 1.0, slowest = 0.0.",
  },
  {
    id: "force_attrition",
    num: 3,
    ...
    unitOfMeasure:
      "Personnel and equipment losses as percentage of starting combat power.",
    benchmark: "Higher remaining combat power ratio = more favorable.",
    scoringGuide:
      "Score 0.0 to 1.0. Lowest attrition (highest remaining combat power) = 1.0, highest attrition = 0.0.",
  },
  {
    id: "concentration",
    num: 4,
    ...
    weight: 3,
    definition:
      "The convergence of appropriate military capability in time and space to create decisive effects against the adversary while presenting multiple dilemmas.",
    unitOfMeasure:
      "Combat power ratio at decisive points during critical phases.",
    benchmark:
      "Sufficient combat power concentrated at decisive points to achieve local superiority while maintaining economy of force elsewhere. Compare across COAs, higher ratio = 1, smaller ratio = 0.",
    scoringGuide:
      "Score 0.0 to 1.0. Best concentration at decisive points = 1.0, worst = 0.0.",
  },
];

export const COA_EVAL_CRITERIA_TEXT = COA_EVAL_CRITERIA.map(
  (c) =>
    `CRITERION ${c.num}: ${c.title} (Weight: ${c.weight})\n  Definition: ${c.definition}\n  Unit of Measure: ${c.unitOfMeasure}\n  Benchmark: ${c.benchmark}\n  Scoring: ${c.scoringGuide}`,
).join("\n\n");

export const TOTAL_CRITERIA_WEIGHT = COA_EVAL_CRITERIA.reduce(
  (sum, c) => sum + c.weight,
  0,
);
