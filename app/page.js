...
const BUILT_IN_DOCTRINE = [
  ...
  {
    name: "ATP 2-01.3 — Intelligence Preparation of the Battlefield",
    tag: "CODED",
  },
  ...
  {
    name: "COA 1 Products — Scheme of Maneuver, Fires, Protection, Sync Matrix",
    tag: "CODED",
  },
  {
    name: "COA 2 Products — Dispersed WGX, Fires, Protection, Sync Matrix",
    tag: "CODED",
  },
  {
    name: "Wargaming Package — Enemy Capabilities, Attrition, BDA Framework",
    tag: "CODED",
  },
];
...
const readFileAsText = (file) =>
  new Promise((res, rej) => {
    ...
    if (file.name.toLowerCase().endsWith(".pdf")) {
      rej(
        new Error(
          `PDF files must be converted to .txt before uploading. "${file.name}" is a PDF. Use a PDF-to-text tool or copy/paste the content into a .txt file.`,
        ),
      );
      return;
    }
    ...
    r.onload = () => {
      ...
      if (typeof text !== "string") {
        rej(
          new Error(
            `"${file.name}" appears to be a binary file. Convert to .txt first.`,
          ),
        );
        return;
      }
      ...
      if (binaryChars > text.length * 0.1) {
        rej(
          new Error(
            `"${file.name}" contains binary data. Convert to .txt first.`,
          ),
        );
        return;
      }
      if (text.length > MAX_DOC_SIZE) {
        rej(
          new Error(
            `"${file.name}" is too large (${(text.length / 1024 / 1024).toFixed(1)}MB). Max is 8MB text. Try splitting the file.`,
          ),
        );
        return;
      }
...
async function callAgent(
  ...
  const sysPrompt =
    systemOverride ||
    (role
      ? buildSystemPrompt(role, null, rt, crossRoomContext, mc)
      : "You are a military staff coordinator.");
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      try {
        ...
        return `⚠ ERROR: Server returned non-JSON (possible timeout). Try again.`;
      }
      if (
        data.error &&
      ...
      >
        <div
          style={{
            height: "100%",
            width: `${pct}%`,
            background: "linear-gradient(90deg, #D4A843, #E08830)",
            borderRadius: 2,
...
export default function WarRoom() {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        if (parsed.roomId && parsed.callsign && parsed.roomType)
  ...
  const uploadToLibrary = useCallback(
    async (e, type) => {
      ...
      const existing =
        type === "doctrine" ? library.doctrine : library.scenario;
      ...
      for (let i = 0; i < validFiles.length; i++) {
        ...
        try {
          ...
          await set(ref(db, `library/${type}/${key}`), {
            name: file.name,
            text,
            size: file.size,
            uploadedAt: Date.now(),
            uploadedBy: callsign || "unknown",
          });
        } catch (err) {
  ...
  const deleteFromLibrary = useCallback(async (type, key) => {
    try {
      await remove(ref(db, `library/${type}/${key}`));
    } catch (e) {}
  }, []);
  ...
  const loadSelectedDocs = useCallback(() => {
    const dF = [],
      ...
      sF = [],
      sT = {};
    selectedDocs.forEach((k) => {
      const d = library.doctrine[k];
      if (d) {
        dF.push(d.name);
        dT[sanitizeKey(d.name)] = d.text;
      }
    });
    selectedScenarios.forEach((k) => {
      const d = library.scenario[k];
      if (d) {
        sF.push(d.name);
        sT[sanitizeKey(d.name)] = d.text;
      }
    });
    setDocFiles(dF);
    setDocTexts(dT);
  ...
  const selectAll = (type) => {
    if (type === "doctrine")
      setSelectedDocs(new Set(Object.keys(library.doctrine)));
    ...
    (id, type) => {
      ...
      try {
        localStorage.setItem(
          "mdmp_session",
          JSON.stringify({ roomId: id, callsign, roomType: type }),
        );
      } catch {}
      ...
      const hb = setInterval(
        () =>
          set(
            ref(db, `rooms/${id}/participants/${sessionId}/lastSeen`),
            Date.now(),
          ),
        10000,
      );
      ...
      const unsub1 = onValue(participantsRef, (s) => {
        const v = s.val();
        if (v)
          setParticipants(
            Object.entries(v)
              .filter(([_, p]) => Date.now() - p.lastSeen < 60000)
              .map(([sid, p]) => ({ ...p, sessionId: sid })),
          );
      });
      const unsub2 = onValue(messagesRef, (s) => {
        const v = s.val();
        if (!v) return;
        const g = {};
        Object.values(v).forEach((m) => {
          if (!g[m.channel]) g[m.channel] = [];
          g[m.channel].push(m);
        });
        Object.keys(g).forEach((c) =>
          g[c].sort((a, b) => a.timestamp - b.timestamp),
        );
        setMessages(g);
      });
      const unsub3 = onValue(mdmpStateRef, (s) => {
        const v = s.val();
        if (v) {
      ...
      const unsub4 = onValue(documentsRef, (s) => {
        const v = s.val();
        if (v) {
          if (v.docFiles) setDocFiles(v.docFiles);
          if (v.scenarioFiles) setScenarioFiles(v.scenarioFiles);
        }
      });
      // Knowledge Base listener — syncs condensed outputs from all participants
      const unsub5 = onValue(knowledgeRef, (s) => {
        const v = s.val();
        if (v) setKnowledgeBase(v);
      });
  ...
  const postMsg = useCallback(
    (ch, sender, color, text, isAgent = false) => {
      if (!roomId) return;
      // Truncate messages to 6K chars max to avoid Firebase "Write too large" errors
      const MAX_MSG = 6000;
      const truncText =
        text.length > MAX_MSG
          ? text.slice(0, MAX_MSG) +
            "\n\n[... Full output in staff channel ...]"
          : text;
      push(ref(db, `rooms/${roomId}/messages`), {
        ...
        console.warn("Firebase write warning:", err.message?.slice(0, 100)),
      );
    },
    [roomId, sessionId],
  );

  const syncMdmpState = useCallback(
    (u) => {
      if (!roomId) return;
      // Only sync metadata to Firebase — stepOutputs stay in local state to avoid "Write too large"
      set(ref(db, `rooms/${roomId}/mdmpState`), {
        currentStep: u.currentStep ?? currentStep,
        isRunning: u.isRunning ?? isRunning,
        completedSteps: u.completedSteps
          ? [...u.completedSteps]
          : [...completedSteps],
      }).catch((err) =>
        console.warn(
          "Firebase state sync warning:",
  ...
  const saveToKnowledgeBase = useCallback(
    (stepId, stepNum, agentId, fullText) => {
      ...
      const summary =
        fullText.length > KB_SUMMARY_LEN
          ? fullText.slice(0, KB_SUMMARY_LEN) + "..."
          : fullText;
      set(ref(db, `rooms/${roomId}/knowledge/${stepId}/${agentId}`), {
        ...
        console.warn("KB write warning:", err.message?.slice(0, 100)),
      );
    },
    [roomId],
  );
  ...
  const fetchCrossRoomContext = useCallback(async () => {
    ...
    for (const rid of otherRooms) {
      try {
        ...
        const msgs = Object.values(msgVal)
        ...
        msgs.forEach((m) => {
          const t =
            m.text?.length > 800 ? m.text.slice(0, 800) + "..." : m.text;
        ...
        if (kbVal) {
          ...
          Object.entries(kbVal).forEach(([, agents]) => {
            Object.entries(agents).forEach(([, data]) => {
              if (data.summary)
                context += `  ${data.agentShort}: ${data.summary.slice(0, 500)}...\n`;
            });
          });
        }
      } catch (err) {
  ...
  const gatherAllRoomWargamingData = useCallback(async () => {
    ...
    for (const rid of rooms) {
      ...
      try {
        ...
        const msgVal = msgSnap.val();
        if (!msgVal) {
          fused += "(No messages in this room)\n";
        } else {
          const msgs = Object.values(msgVal).sort(
        ...
        fused += `(Error fetching: ${err.message})\n`;
      }
    }
    return fused;
  }, []);
  ...
  const runCoaComparison = useCallback(async () => {
    ...
    try {
      ...
      postMsg(
        ...
        "SYSTEM",
        "#D4A843",
        `✓ Data fused from all rooms. Running weighted comparison against CDR criteria...`,
        true,
      );
      ...
      const result = await callAgent("xo", comparisonPrompt, {
        maxTokens: 4000,
        retries: 2,
        roomType: "OVERALL",
        systemOverride: baseSystem + criteriaEnd,
      });
      // Step 4: Post results
      if (result && !result.startsWith("⚠")) {
        ...
        postMsg(
          ...
          "25 ID XO",
          "#D4A843",
          `⚠ COA COMPARISON FAILED\n\n${result || "Unknown error — try again in 30 seconds (API rate limit may apply)."}\n\nData was gathered successfully. The comparison agent failed. Try running again.`,
          true,
        );
      }
    } catch (err) {
      postMsg(
        ...
        "SYSTEM",
        "#E05555",
        `⚠ COA COMPARISON ERROR: ${err.message}`,
        true,
      );
    }
    setIsRunning(false);
    ...
    async (e, type) => {
      ...
      for (let i = 0; i < validFiles.length; i++) {
        ...
        try {
          ...
          if (type === "doctrine") {
            if (!nDF.includes(file.name)) nDF.push(file.name);
            nDT[sanitizeKey(file.name)] = text;
          } else {
            if (!nSF.includes(file.name)) nSF.push(file.name);
            nST[sanitizeKey(file.name)] = text;
          }
        } catch (err) {
          errors.push(`${file.name}: ${err.message}`);
        }
      ...
      setDocFiles(nDF);
      setDocTexts(nDT);
      setScenarioFiles(nSF);
      setScenarioTexts(nST);
      // Only sync file names to Firebase (not full text) to avoid "Write too large"
      if (roomId)
        set(ref(db, `rooms/${roomId}/documents`), {
          docFiles: nDF,
          scenarioFiles: nSF,
        });
      e.target.value = "";
  ...
  const getDocContext = useCallback(() => {
    ...
    let c = "";
    if (Object.keys(scenarioTexts).length > 0) {
      c += `[${Object.keys(scenarioTexts).length} scenario documents loaded — content provided via SCENARIO_PACKAGE in prompt]\n\n`;
    }
    if (Object.keys(docTexts).length > 0) {
      c += "══ DOCTRINE REFERENCES ══\n\n";
      Object.entries(docTexts).forEach(([n, t]) => {
        c += `── ${n} ──\n${t}\n\n`;
      });
    }
    return c;
  ...
  const getPrevOutputs = useCallback(() => {
    let p = "";
    Object.entries(stepOutputs).forEach(([sid, outs]) => {
      const st = MDMP_STEPS.find((s) => s.id === sid);
      if (st) {
        p += `\n══ STEP ${st.num}: ${st.title} ══\n`;
        Object.entries(outs).forEach(([rid, txt]) => {
          const r = STAFF[rid];
          if (r) p += `\n── ${r.title} ──\n${txt}\n`;
        });
      }
  ...
  const runStep = useCallback(
    async (si) => {
      ...
      setIsRunning(true);
      setCurrentStep(si);
      syncMdmpState({ isRunning: true, currentStep: si });
      const docs = getDocContext(),
        prev = getPrevOutputs(),
        prompt = step.prompt(docs, prev);
      ...
      const allAgents = [...step.lead, ...step.support],
        results = {};
      ...
      for (const a of allAgents) {
        if (stopRef.current) {
          postMsg(
            ...
            "SYSTEM",
            "#E05555",
            "⛔ Step execution HALTED by operator.",
            true,
          );
          break;
        }
        ...
        const sysOverride = criteriaOverride
          ? buildSystemPrompt(agent, null, roomType) + criteriaOverride
          : undefined;
        const result = await callAgent(
          a,
          `${prompt}\n\nYou are the ${agent.title}. Provide YOUR specific outputs for this step. ${stepInstruction}`,
          { maxTokens: stepMaxTokens, roomType, systemOverride: sysOverride },
        );
        if (stopRef.current) {
          postMsg(
            "cop",
      ...
      postMsg(
        ...
        "SYSTEM",
        "#D4A843",
        "All sections reported. XO synthesizing...",
        true,
      );
      ...
      try {
        ...
        const xoInstruction = isOrdersStep
          ? `Compile the base OPORD body (Situation, Mission, Execution, Sustainment, Command & Signal) and reference each staff annex.`
          : `BLUF, key issues, gaps, risks. Under 400 words.`;
        xo = await callAgent(
          "xo",
          `XO: Synthesize Step ${step.num}.\n\n${xoInputs}\n\n${xoInstruction}`,
          { maxTokens: xoMaxTokens, retries: 1, roomType },
        );
      } catch (e) {
        xo = null;
      }
      ...
      if (xo && !xo.startsWith("⚠")) {
        ...
        saveToKnowledgeBase(
          ...
          step.num,
          "xo_synthesis",
          `STEP ${step.num} XO SYNTHESIS:\n${xo}`,
        );
      } else {
        const completed = Object.entries(results)
          .filter(([_, t]) => typeof t === "string" && !t.startsWith("⚠"))
          .map(([id]) => STAFF[id]?.short || id);
      ...
      postMsg(
        ...
        "SYSTEM",
        "#D4A843",
        `✓ Step ${step.num} COMPLETE\n${step.outputs.map((o) => `  ✓ ${o}`).join("\n")}\n\nReview outputs, interact with any section, or proceed.`,
        true,
      );
      const nO = { ...stepOutputs, [step.id]: results },
        nC = new Set([...completedSteps, si]);
      setStepOutputs(nO);
      setCompletedSteps(nC);
      setIsRunning(false);
  ...
  const sendMessage = useCallback(async () => {
    ...
    const coaComparePattern =
      /\b(coa\s*compar|compare\s*coa|run\s*coa|decision\s*matrix|weighted\s*compar|eval\s*criteria)\b/i;
    if (
      roomType === "OVERALL" &&
      activeChannel === "cop" &&
      coaComparePattern.test(text)
    ) {
      postMsg(
        "cop",
        "25 ID XO",
        "#D4A843",
        "Roger. Initiating COA Comparison with CDR evaluation criteria. Fusing data from all rooms...",
        true,
      );
      await runCoaComparison();
      return;
    ...
    let crossCtx = "";
    try {
      crossCtx = await fetchCrossRoomContext();
    } catch {}
    // Detect if all MDMP steps are complete — switch to execution/sim support mode
    const allMdmpDone = completedSteps.size >= MDMP_STEPS.length;
    const prevContext = allMdmpDone
      ? `MDMP STATUS: ALL 7 STEPS COMPLETE. OPORD PUBLISHED. NOW IN EXECUTION.\n\nCOMPLETED STAFF PRODUCTS (reference only -- do NOT restart any step):\n${getPrevOutputs()}`
      : `PREV OUTPUTS:\n${getPrevOutputs()}`;
    if (STAFF[activeChannel]) {
      const agent = STAFF[activeChannel];
      postMsg(activeChannel, agent.title, agent.color, "Processing...", true);
      const recent = (messages[activeChannel] || [])
        .slice(-20)
        .map((m) => `${m.sender}: ${m.text}`)
        .join("\n\n");
      const execInstruction = allMdmpDone
        ? `Respond as ${agent.title} in EXECUTION MODE. Answer the question directly. Do NOT offer to run MDMP steps. All planning is complete. Focus on sim support, tactical analysis, and current operations.`
      ...
      const resp = await callAgent(
        ...
        `CONVERSATION:\n${recent}\n\nDOCS:\n${getDocContext()}\n\n${prevContext}\n\n${sn} says: "${text}"\n\n${execInstruction}`,
        {
          maxTokens: 16000,
          roomType,
          crossRoomContext: crossCtx,
          mdmpComplete: allMdmpDone,
        },
      );
      postMsg(activeChannel, agent.title, agent.color, resp, true);
    } else if (activeChannel === "cop") {
      const execCop = allMdmpDone
        ? `As 25 ID XO in EXECUTION MODE, respond. All MDMP is complete. Do NOT offer to run planning steps. Focus on current operations and sim support.`
        : `As 25 ID XO, respond.`;
      const resp = await callAgent(
        "xo",
        `CDR (${sn}) posted to COP: "${text}"\n\nRecent COP:\n${(
          messages.cop || []
        )
          .slice(-10)
      ...
      postMsg(
        ...
        "SYSTEM",
        "#FFD700",
        "CDR guidance recorded. Staff notified.",
        true,
      );
      postMsg(
        "cop",
  ...
  const exportBrief = useCallback(
    async (type) => {
      setExporting(true);
      setShowExport(true);
      setExportText("Generating brief...");
      try {
        ...
        try {
          ...
          setExportText(
            "Error: Server returned non-JSON (possible timeout). Try again.",
          );
          setExporting(false);
          return;
        }
  ...
  // ═════════ SETUP ═════════
  if (phase === "setup") {
    const FIXED_ROOMS = Object.values(ROOM_TYPES).filter(
      (r) => !["REDCELL1", "REDCELL2", "COA1", "COA2"].includes(r.id),
    );
    return (
      <div style={S.root}>
        <div style={S.over}>
          <div style={{ ...S.box, maxWidth: 520 }}>
            ...
            <div style={S.sub}>
              25th Infantry Division "Tropic Lightning" — Virtual Staff Platform
            </div>
            {savedSession && savedSession.roomType && (
              ...
              >
                ...
                <div
                  style={{ fontSize: 12, color: "#C0CCD8", marginBottom: 8 }}
                >
                  Room{" "}
                  <span
                    style={{
                      color:
                        ROOM_TYPES[savedSession.roomType]?.color || "#D4A843",
                      fontWeight: 700,
                    }}
                  >
                    {ROOM_TYPES[savedSession.roomType]?.shortName ||
                      savedSession.roomId}
                  </span>{" "}
                  as{" "}
                  <span style={{ color: "#4A9EE8", fontWeight: 700 }}>
                    {savedSession.callsign}
                  </span>
                </div>
                <div style={{ display: "flex", gap: 8 }}>
                  <button
                    style={{ ...S.btn("#4A9EE8", false), flex: 1 }}
                    onClick={() => {
                      setCallsign(savedSession.callsign);
                      joinRoom(savedSession.roomId, savedSession.roomType);
                      setPhase("lobby");
                    }}
                  >
                    RESUME SESSION
                  ...
                  <button
                    ...
                    onClick={() => {
                      try {
                        localStorage.removeItem("mdmp_session");
                      } catch {}
            ...
            <input
              style={S.ti}
              placeholder="e.g. LIGHTNING 6, MAJ Smith"
              value={callsign}
              ...
              {FIXED_ROOMS.map((room) => (
                <button
                  ...
                  style={{
                    ...
                    gap: 14,
                    padding: "16px 20px",
                    background: `${room.color}30`,
                    border: `2px solid ${room.color}`,
                    borderRadius: 8,
                    cursor: callsign ? "pointer" : "default",
                    opacity: callsign ? 1 : 0.4,
                    textAlign: "left",
                    fontFamily: "inherit",
                    transition: "all 0.15s",
                    boxShadow: `0 0 20px ${room.color}55, inset 0 0 30px ${room.color}15`,
                  }}
                >
                  <span
                    style={{
                      ...
                      textAlign: "center",
                      flexShrink: 0,
                      textShadow: `0 0 12px ${room.color}, 0 0 24px ${room.color}88`,
                    }}
                  >
                    {room.icon}
                  </span>
                  <div>
                    <div
                      style={{
                        ...
                        color: "#FFFFFF",
                        letterSpacing: 0.5,
                        textShadow: `0 0 8px ${room.color}AA`,
                      }}
                    >
                      {room.name}
                    </div>
      ...
      <div style={S.root}>
        <div style={S.over}>
          <div style={{ ...S.box, maxWidth: 640 }}>
            ...
            <div style={{ fontSize: 11, color: "#566A80", marginBottom: 16 }}>
              Upload once, use forever. Duplicates are automatically blocked.
            </div>
              ...
              <div key={type} style={{ marginBottom: 20 }}>
                <span
                  style={{
                    ...
                    fontWeight: 700,
                    letterSpacing: 1,
                    color: type === "doctrine" ? "#4A9EE8" : "#E05555",
                    textTransform: "uppercase",
                  }}
                >
                  {type} ({Object.keys(library[type]).length})
                </span>
                {Object.entries(library[type]).map(([key, doc]) => (
                  ...
                  >
                    ...
                    <span style={{ flex: 1, fontSize: 11, color: "#B8C4D4" }}>
                      {doc.name}
                    </span>
                    <span style={{ fontSize: 9, color: "#566A80" }}>
                    ...
                    <button
                      ...
                      onClick={() => deleteFromLibrary(type, key)}
                    >
                      ✕
                    </button>
                  </div>
                ...
                <div style={{ position: "relative", marginTop: 6 }}>
                  <input
                    ...
                    onChange={(e) => uploadToLibrary(e, type)}
                    style={{
                      position: "absolute",
                      inset: 0,
                      opacity: 0,
                  ...
                  <div style={S.ub}>
                    + Upload{" "}
                    {type === "doctrine" ? "Doctrine" : "Scenario Docs"}
                  </div>
                </div>
              </div>
            ))}
  ...
  if (phase === "lobby") {
    ...
    return (
      <div style={S.root}>
        <div style={S.over}>
          <div style={{ ...S.box, maxWidth: 600 }}>
            <div style={S.logo}>
              ⚔ {rc.icon} {rc.shortName || roomId}
            </div>
            <div style={{ ...S.sub, color: rc.color || "#566A80" }}>
              {rc.name || roomId}
            </div>
            <div
              style={{
                ...
                background: "#0A0E14",
                borderRadius: 4,
                border: `1px solid ${rc.color || "#1E2A3A"}33`,
                marginBottom: 12,
                fontSize: 11,
                color: "#7A8A9E",
              }}
            ...
            >
              {participants.map((p) => (
                <span key={p.sessionId} style={S.pt}>
                  <span style={S.dot} /> {p.callsign}
                </span>
              ))}
            </div>
            ...
            >
              ...
              >
                NATIVE STAFF DOCUMENTS
              </div>
              <div style={{ fontSize: 10, color: "#566A80", marginBottom: 10 }}>
                Scenario and doctrine are coded into the virtual staff. No
                upload required.
              </div>
              ...
              <div>
                ...
                >
                  DOCTRINE ({BUILT_IN_DOCTRINE.length} publications)
                </span>
                {BUILT_IN_DOCTRINE.map((d) => (
                  <div
                    key={d.name}
                    style={{
                      display: "flex",
            ...
            >
              Additional documents (SIGACTs, SITREPs, etc.) can be uploaded
              during the exercise.
            </div>

            <button
              style={{
                ...S.btn(rc.color || "#D4A843", false),
                width: "100%",
                marginTop: 12,
                padding: 14,
              }}
    ...
    showExport ? (
      ...
      >
        ...
        >
          ...
          >
            <span style={{ color: "#D4A843", fontWeight: 700, fontSize: 14 }}>
              📄 EXPORTED BRIEF
            </span>
            <div style={{ display: "flex", gap: 8 }}>
              <button
              ...
              <button
                ...
                onClick={() => {
                  const b = new Blob([exportText], { type: "text/plain" });
                  const u = URL.createObjectURL(b);
                  const a = document.createElement("a");
                  a.href = u;
                  a.download = "25ID_Brief.txt";
                  a.click();
                }}
              >
                DOWNLOAD
          ...
          >
            {exporting ? (
              <>
                <Spinner /> Generating brief from MDMP outputs...
              </>
            ) : (
              exportText
            )}
  ...
  return (
    <div style={S.root}>
      ...
      <div style={S.hdr}>
        <div style={S.hdrL}>
          <span style={S.hdrLogo}>⚔ 25 ID WAR ROOM</span>
          <span
            style={{
              ...S.hdrM,
              color: ROOM_TYPES[roomType]?.color || "#566A80",
            }}
          >
            {ROOM_TYPES[roomType]?.icon}{" "}
            {ROOM_TYPES[roomType]?.shortName || roomId}
          </span>
        </div>
        <div style={S.hdrR}>
          <button
            style={{
              background: showMap ? "#D4A843" : "#1A2332",
              color: showMap ? "#0A0E14" : "#D4A843",
      ...
      <div style={S.body}>
        ...
        <div style={S.sb}>
          ...
          {channelList.slice(0, 2).map((c) => (
            <button
              key={c.id}
              style={S.cb(activeChannel === c.id, c.color)}
          ...
          {channelList.slice(2).map((c) => (
            ...
            >
              ...
              {STAFF[c.id]?.short} — {c.wff?.split("/")[0]}
            </button>
          ))}
          <div style={{ flex: 1 }} />
        </div>
          ...
          {showMap ? (
            ...
            <>
              <div style={S.tb}>
                <span style={S.ct}>
                  {aStaff
                    ? `${aStaff.icon} ${aStaff.title}`
                    : channelList.find((c) => c.id === activeChannel)?.name}
                </span>
                {aStaff && (
                  <span
                    style={{ fontSize: 11, color: "#566A80", marginLeft: 12 }}
                  >
                    {aStaff.desc}
                  </span>
                )}
              ...
              <div style={S.ma}>
                {chMsgs.map((m, idx) => {
                  ...
                  const displayColor =
                    m.senderColor ||
                    (m.role === "user" ? "#D4A843" : "#7A8A9E");
                  const isHuman = m.isHuman || m.role === "user";
                  return (
                    <div
                      key={m.id || `msg-${idx}`}
                      style={S.msg(isHuman, displayColor)}
                    >
                      <div style={S.av(displayColor)}>
                        {isHuman ? "👤" : displayName.slice(0, 2).toUpperCase()}
                      </div>
              ...
              <div style={S.ib}>
                <textarea
                  style={S.inf}
                  placeholder={`Message as ${callsign}... (Shift+Enter for newline)`}
                  value={inputText}
                ...
                />
                <button
                  style={S.btn("#4A9EE8", !inputText.trim() || isRunning)}
        ...
        <div style={S.rp}>
          ...
          <div style={{ ...S.rps, flex: 1, overflowY: "auto" }}>
            ...
            <button
              ...
              style={S.collapseBtn}
            >
              <span style={{ color: "#E05555", fontSize: 10 }}>
                {scenarioOpen ? "▼" : "▶"}
              </span>
              <span
                style={{
              ...
              >
                SCENARIO ({BUILT_IN_SCENARIO.length})
              </span>
            </button>
            {scenarioOpen &&
              BUILT_IN_SCENARIO.map((d) => (
            ...
            <button
              ...
              style={{ ...S.collapseBtn, marginTop: 6 }}
            >
              <span style={{ color: "#4A9EE8", fontSize: 10 }}>
                {doctrineOpen ? "▼" : "▶"}
              </span>
              <span
                style={{
              ...
              >
                DOCTRINE ({BUILT_IN_DOCTRINE.length})
              </span>
            </button>
            {doctrineOpen &&
              BUILT_IN_DOCTRINE.map((d) => (
            ...
            {(docFiles.length > 0 || scenarioFiles.length > 0) && (
              ...
              >
                ...
                >
                  UPLOADED ({docFiles.length + scenarioFiles.length})
                </span>
                {scenarioFiles.map((f) => (
                  <div key={f} style={S.ft}>
                    📋 {f}
                  </div>
                ))}
                {docFiles.map((f) => (
            ...
            <div style={{ position: "relative", marginTop: 8 }}>
              <input
                ...
                multiple
                accept=".txt,.md,.doc,.docx,.pdf"
                onChange={(e) => handleUpload(e, "doctrine")}
                style={{
                  position: "absolute",
                  inset: 0,
                  opacity: 0,
          ...
          {(completedSteps.size > 0 ||
            ...
            >
              <button onClick={() => setKbOpen(!kbOpen)} style={S.collapseBtn}>
                ...
                <span style={S.rpt}>
                  KNOWLEDGE BASE ({Object.keys(knowledgeBase).length} steps)
                </span>
              </button>
              {kbOpen &&
                (() => {
                  ...
                  const sortedSteps = MDMP_STEPS.filter((s) =>
                    allStepIds.has(s.id),
                  );
                  return sortedSteps.map((step) => {
                    ...
                    return (
                      <div key={step.id} style={{ marginBottom: 6 }}>
                        ...
                        >
                          <span style={{ color: "#D4A843", fontSize: 9 }}>
                            {isExpanded ? "▼" : "▶"}
                          </span>
                          <span
                            style={{
                            ...
                            <div style={{ marginBottom: 4 }}>
                              {step.outputs.map((o) => (
                                <div
                                  ...
                                  style={{
                                    fontSize: 8,
                                    color: completedSteps.has(
                                      MDMP_STEPS.indexOf(step),
                                    )
                                      ? "#3EAF5C"
                                      : "#566A80",
                                    paddingLeft: 4,
                                ...
                                >
                                  {completedSteps.has(MDMP_STEPS.indexOf(step))
                                    ? "✓"
                                    : "○"}{" "}
                                  {o}
                                </div>
              ...
              {roomType === "OVERALL" && (
                <>
                  ...
                  >
                    Fuses wargaming data from all rooms. Scores COA 1 vs COA 2
                    against CDR criteria: WGX Speed (w:2), Tempo (w:1), Force
                    Attrition (w:3), Concentration (w:3).
                  </div>
                </>
              )}
              ...
              <button
                ...
                disabled={exporting}
                onClick={() => exportBrief("decision_brief")}
              >
                📄 CDR Decision Brief
              </button>
              <button
                style={{
                  ...S.btn("#4A9EE8", exporting),
                  width: "100%",
                  padding: 6,
                  fontSize: 9,
                  marginBottom: 4,
                ...
                disabled={exporting}
                onClick={() => exportBrief("opord_summary")}
              >
                📋 OPORD Summary
              </button>
              <button
                style={{
                  ...S.btn("#3EAF5C", exporting),
                  width: "100%",
                  padding: 6,
                  fontSize: 9,
                }}
                disabled={exporting}
                onClick={() => exportBrief("staff_summary")}
              >
                📊 Staff Summary
              </button>
            </div>
...
const S = {
  ...
  hdr: {
    background: "linear-gradient(180deg,#131920,#0D1117)",
    borderBottom: "1px solid #1E2A3A",
    padding: "8px 16px",
    display: "flex",
    alignItems: "center",
  ...
  cb: (a, c) => ({
    ...
    padding: "6px 12px",
    cursor: "pointer",
    background: a ? "#1A2332" : "transparent",
    color: a ? "#E8ECF2" : "#7A8A9E",
    fontSize: 12,
    ...
    textAlign: "left",
    fontFamily: "inherit",
    borderLeft: `2px solid ${a ? c : "transparent"}`,
  }),
  main: {
    flex: 1,
    display: "flex",
  ...
  msg: (h, c) => ({
    ...
    padding: "8px 12px",
    borderRadius: 4,
    background: h ? "#131D2A" : "#0F1520",
    borderLeft: `3px solid ${h ? "#4A9EE8" : c}`,
  ...
  stb: (a, d) => ({
    ...
    textAlign: "left",
    fontFamily: "inherit",
    color: d ? "#3EAF5C" : a ? "#D4A843" : "#566A80",
    fontSize: 11,
  }),
  stn: (a, d) => ({
    width: 18,
    height: 18,
    borderRadius: "50%",
    border: `1.5px solid ${d ? "#3EAF5C" : a ? "#D4A843" : "#2A3A4A"}`,
    display: "flex",
    alignItems: "center",
    justifyContent: "center",
    fontSize: 9,
    fontWeight: 700,
    background: d ? "#3EAF5C22" : "transparent",
    flexShrink: 0,
  }),
  ft: {
    display: "flex",
  ...
  btn: (c, d) => ({
    background: d ? "#1A2332" : c,
    color: d ? "#566A80" : "#0A0E14",
    border: "none",
    borderRadius: 4,
    padding: "8px 14px",
    fontWeight: 700,
